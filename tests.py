...
import ttext, sys, os, json, argparse, re
...
for section in extractor_tests.get('tests'):
    ...
    for test in extractor_tests.get('tests').get(section):
        ...
        if (args.ignore_narrow_errors or narrow_build) and section_tag and description_match:
            ...
            sys.stdout.flush()
            continue
        extractor = ttext.extractor.Extractor(test.get('text'))
        if section == 'mentions':
...
for section in autolink_tests.get('tests'):
    ...
    for test in autolink_tests.get('tests').get(section):
        if (args.ignore_narrow_errors or narrow_build) and section in ['hashtags'] and test.get('description') in [
            ...
            sys.stdout.flush()
            continue
        autolink = ttext.autolink.Autolink(test.get('text'))
...
if validate_tests:
    ...
    for section in validate_tests.get('tests'):
        ...
        for test in validate_tests.get('tests').get(section):
            validator = ttext.validation.Validation(test.get('text'))
            if section == 'tweets':
